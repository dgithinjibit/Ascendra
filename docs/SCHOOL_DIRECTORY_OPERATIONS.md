...
The reviewer interface is available at `/internal/school-review` and authenticates through the protected reviewer endpoint. Approve and reject decisions write to `public.school_review_audits` using a deterministic SHA-256 digest over an allowlisted set of school fields. Contact details, learner data, raw submissions, and raw reviewer identifiers are excluded from the audit record; reviewer references are hashed before storage. The audit digest is returned to the console for operator evidence. Until a request is approved, it does not unblock student school selection.
