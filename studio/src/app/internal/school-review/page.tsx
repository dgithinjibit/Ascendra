...
export default function SchoolReviewPage() {
  ...
  const decide = async (requestId: string, action: 'approve' | 'reject') => {
    setLoading(true); setMessage(null);
    try {
      const response = await fetch('/api/internal/schools/onboarding', { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-syncsenta-school-review-token': token, ...(operatorRef.trim() ? { 'x-syncsenta-reviewer-ref': operatorRef.trim() } : {}) }, body: JSON.stringify({ requestId, action }) });
      const result = await response.json();
      ...
      setMessage(`${action === 'approve' ? 'School published to the active directory.' : 'Registration rejected.'} Audit digest: ${result.auditDigest || 'unavailable'}`);
    ...
    <main className="education-shell flex justify-center p-5">
      <Card className="w-full max-w-4xl border-border/80 shadow-[0_18px_45px_hsl(174_30%_16%/0.1)]">
        ...
        <CardContent className="space-y-5">
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-end"><div className="flex-1 space-y-2"><Label htmlFor="review-token">Review token</Label><Input id="review-token" type="password" autoComplete="off" value={token} onChange={(event) => setToken(event.target.value)} placeholder="Enter the operator review token" /></div><div className="flex-1 space-y-2"><Label htmlFor="operator-ref">Operator reference (optional)</Label><Input id="operator-ref" autoComplete="off" value={operatorRef} onChange={(event) => setOperatorRef(event.target.value)} placeholder="e.g. Daniel / reviewer-1" /></div><Button onClick={load} disabled={!token || loading}>{loading ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Loading…</> : 'Load pending requests'}</Button></div>
